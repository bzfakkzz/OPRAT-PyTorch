...
class TorchLeNet5(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.conv1 = nn.Conv2d(1, 6, kernel_size=5, padding=2).to(device)
        self.pool1 = nn.AvgPool2d(kernel_size=2, stride=2).to(device)
        self.conv2 = nn.Conv2d(6, 16, kernel_size=5, padding=2).to(device)
        self.pool2 = nn.AvgPool2d(kernel_size=2, stride=2).to(device)
        self.flatten = nn.Flatten().to(device)
        self.fc1 = nn.Linear(784, 120).to(device)  # 16*7*7=784
